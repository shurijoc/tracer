...
### Changed
- **Plugin directory submission prep — second pass (#6).** Added two new det tests to mechanically gate review checks 2 & 3: `skill_referenced_files_exist` walks every `$TRACER_DIR/{scripts,templates}/<name>` reference in SKILL.md and verifies the file is bundled (catches typos / forgotten renames that would only surface at install time), and `skill_no_out_of_plugin_writes` greps for any absolute path under `/etc /usr /var /private /System` to catch accidental out-of-directory write instructions. SKILL.md frontmatter `description` now leads with an English summary (JP triggers preserved verbatim) so a directory reviewer can read the contract without Japanese. `claude plugin validate . --strict` ✔, det 14/14, behavioral 12/12.
- **Dashboard polish (#1).** Reworked `templates/dashboard-template.html` for scannability: action banner now has a color-coded letter chip (A/B = accent, C = warn, D/E = ok) instead of a uniform blue block; sparkline gains a highlighted latest-value marker with inline label + a `direction badge` (`改善中 ↓` / `停滞 →` / `悪化 ↑`) computed from the recent slope; intervention list items take a `kind` badge (`escalation` / `advisor` / `merge`) so the human can scan reason-for-attention at a glance; frozen-issues empty state has its own muted styling; schedule section drops the redundant `now` row (already shown in the banner). Typography density bumped (body 15→14.5, metric values 20→24, section heads 15→13.5 with tighter tracking). New placeholders documented in SKILL.md Step 3: `{{ACTION_LETTER_CLASS}}`, `{{LATEST_X/Y}}`, `{{LATEST_LABEL_Y}}`, `{{METRIC_DIRECTION_CLASS/LABEL}}`. Self-contained / zero-JS / `file://`-openable contract preserved. det 12/12, behavioral 12/12.
